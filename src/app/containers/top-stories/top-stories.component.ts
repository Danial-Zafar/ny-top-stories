...
export class TopStoriesComponent implements OnInit  {
  ...
  this.apiService.get(AppApiUrls.NY.getStoriesByCategory( storyCategory, environment.api_key ))
    .subscribe({
      next: (response : any) => {
  ...
  searchStories(text:string){
    if(this.searchTxt==""){
      ...
      this.apiService.get(AppApiUrls.NY.searchStoriesByText( text, environment.api_key ))
      .subscribe({
        next: (response : any) => {
          this.topStories = response.response.docs;
            this.storeService.setStories(this.topStories);

            if (!this.searchHistory.some(q => q === this.searchTxt) && this.searchTxt!="") {
